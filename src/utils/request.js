...
instance.interceptors.request.use(
  (requestConf) => {
    // 是否需要设置 token
    const isToken = (requestConf.headers || {}).isToken === false
    // if(requestConf.method.toUpperCase() == 'PUT') {
    //   requestConf.method='POST'
    //   requestConf.headers['X-HTTP-Method-Override']="PUT"
    // }
    // if(requestConf.method.toUpperCase() == 'DELETE') {
    //   requestConf.method='POST'
    //   requestConf.headers['X-HTTP-Method-Override']="DELETE"
    // }
    if (getToken() && !isToken) {
      requestConf.headers['Authorization'] = 'Bearer ' + getToken() // 让每个请求携带自定义token 请根据实际情况自行修改
    }
    // get请求映射params参数
    if (requestConf.method === 'get' && requestConf.params) {
      let url = requestConf.url + '?'
      for (const propName of Object.keys(requestConf.params)) {
        const value = requestConf.params[propName]
