 ...
 */
import Vue from 'vue'
import {getUserInfo, login, logout, socialLogin} from '@/api/common/user'
...
const actions = {
  ...
  async login({commit}, userInfo) {
    const {[tokenName]: token,} = await login(userInfo)
    if (token) {
  ...
  async getUserInfo({commit, dispatch}) {
    ...
    if (
      ...
      if (user) commit('setUsername', user.nickName)
      // 如不使用avatar头像,可删除以下代码
      if (user) commit('setAvatar', user.avatar)
