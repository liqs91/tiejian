<template>
  <div class="app-container">
    ...
    <vab-query-form>
      <vab-query-form-left-panel :span="12">
        <el-form :inline="true" :model="queryForm" @submit.native.prevent>
          <el-form-item>
            <el-input v-model.trim="queryForm.roleName" clearable
                      placeholder="请输入角色名称"/>
          </el-form-item>
      ...
      <vab-query-form-right-panel :span="12">
        <el-button v-hasPermi="['system:role:add']"
                   icon="el-icon-plus"
                   style="float: right"
                   plain type="primary"
                   @click="handleAdd">新增
    ...
    <el-table v-loading="loading" border :data="roleList" @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" class-name="fixed-width" label="角色名称" prop="roleName"
      ...
      <el-table-column align="center" class-name="small-padding fixed-width" label="操作" width="300">
        <template slot-scope="scope">
          <el-button v-hasPermi="['system:role:auth']" icon="el-icon-circle-check" size="mini" type="back"
                     @click="handleDataScope(scope.row)">数据权限
          </el-button>
          <el-button v-hasPermi="['system:role:edit']" icon="el-icon-edit" size="mini" type="even"
                     @click="handleUpdate(scope.row)">修改
          </el-button>

          <el-button v-hasPermi="['system:role:remove']" icon="el-icon-delete" size="mini" type="danger"
    ...
    <el-dialog append-to-body :title="title" :visible.sync="open" width="50%">
      <el-form ref="form" label-width="80px" :model="form" :rules="rules">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="角色顺序" prop="roleSort">
              <el-input type="number" v-model="form.roleSort" controls-position="right" :min="0"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-form-item label="菜单权限">
          ...
          <el-tree ref="menu" :check-strictly="!form.menuCheckStrictly" class="tree-border" :data="menuOptions"
                   empty-text="加载中，请稍后" node-key="id" :props="defaultProps" show-checkbox/>
        </el-form-item>
        <el-row>
          <el-col :span="16">
            <el-form-item label="备注">
              <el-input v-model="form.remark" placeholder="请输入内容" :rows="3" type="textarea"/>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listRole(this.addDateRange(this.queryForm, this.dateRange)).then(
        (response) => {
    ...
    handleDataScope(row) {
      this.reset()
      const roleDeptTreeselect = this.getDeptTree(row.roleId)
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$confirm('是否确认删除角色为"' + row.roleName + '"的数据项?',
        '警告',
